...
func (r *StudentsRepo) Add(ctx context.Context, student *model.Student) (*model.Student, error) {
	...
	err := r.db.ExecQueryRow(ctx, `INSERT INTO students(name, course_program) VALUES ($1, $2) RETURNING id, name, course_program, created_at, updated_at`,
		student.Name, student.CourseProgram).Scan(&dbStudent.ID, &dbStudent.Name, &dbStudent.CourseProgram,
