...
func main() {
	...
	for i := 0; i < workerTotal; i++ {
		wg.Add(1)
		go worker(ctx, &wg, server, orders)
	}
...
func worker(ctx context.Context, wg *sync.WaitGroup, server *gateway.Implementation, orders <-chan model.ClientID) {
	defer wg.Done()
	server.PipelineFan(ctx, orders)
}
